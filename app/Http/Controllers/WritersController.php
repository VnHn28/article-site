    ...
    public function show($id) {
		...
		if($checkAdmin){
    	    ...
    	    $articles = \App\tab_articles::select(['tab_articles.*', 'tab_article_categorys.name as category_name'])
                                        ...
                                        ->where('author_id', '=', $id)->where('tab_articles.enable', '=', 1)->where('tab_articles.reviewed', '=', 1)->orderBy('tab_articles.priority', 'DESC')->orderBy('tab_articles.id', 'DESC')->get();
        } else {
        ...
        $recommendationBooks = \App\tab_books::where('recommend_person_author_id' , '=', $id)->where('tab_books.enable', '=', 1)->where('tab_books.reviewed', '=', 1)->orderBy('tab_books.priority', 'DESC')->orderBy('tab_books.id', 'DESC')->get();
        $relatedGroups = \App\tab_groups::select(['tab_groups.*', 'tab_group_categorys.name as category_name','tab_group_article_categories.name as article_category_name', 'tab_group_article_categories.list_color'])
                                        ...
                                        ->where('author_id', '=', $id)->where('tab_groups.enable', '=', 1)->where('tab_groups.reviewed', '=', 1)->orderBy('tab_groups.priority', 'DESC')->orderBy('tab_groups.id', 'DESC')->get();
    	return view('writer')
    			->with('author', $author)
